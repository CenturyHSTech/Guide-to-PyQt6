...
class CustomTitleBar(QWidget):
    def __init__(self, parent):
        ...
        close_icon = QIcon()
        close_icon.addFile("resources/close.svg")  # Close has only a single state.
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        central_styles = "#Container {background: qlineargradient(x1:0"
        central_styles += f"y1:0, x2:1 y2:1, stop:0 {python_blue}"
        central_styles += f"stop:1 {python_light_blue}); border-radius: 5px;"
        central_widget.setStyleSheet(
            """#Container {
            background: qlineargradient(x1:0 y1:0, x2:1 y2:1, stop:0 #306998 stop:1 #4B8BBE);
